...
impl SetupProvider for AssetLoader {
    fn name(&self) -> &'static str { "Asset Loader" }
    
    fn time_estimate(&self) -> f32 { 2.0 } // Takes longer than average
...
impl SetupProvider for SceneBuilder {
    fn name(&self) -> &'static str { "Scene Builder" }
...
impl SetupProvider for PlayerSpawner {
    fn name(&self) -> &'static str { "Player Spawner" }
...
static PROVIDER_REGISTRY: OnceLock<Mutex<HashMap<TypeId, Arc<dyn SetupProvider>>>> = OnceLock::new();
...
impl SetupKey for GameSetupKey {
    fn register_progress_checker(&self, world: &mut World) -> SystemId<(), Progress> {
        ...
        world.register_system(move |world: &World| {
            provider.check_progress(world)
        })
    }
...
fn main() {
    ...
    println!("- {} (time estimate: {:.1}x)", asset_key.name(), asset_key.relative_time_estimate());
    println!("- {} (time estimate: {:.1}x)", scene_key.name(), scene_key.relative_time_estimate());
    println!("- {} (time estimate: {:.1}x)", player_key.name(), player_key.relative_time_estimate());
    println!();
    ...
    app.register_provider(
        load_assets.provides([asset_key.clone()])
    );
    ...
    println!("Dependencies: {} → {} → {}", asset_key.name(), scene_key.name(), player_key.name());
    println!();
